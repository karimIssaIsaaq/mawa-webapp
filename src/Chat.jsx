...
export default function ChatBox() {
  ...
  const handleSend = async (userMessage) => {
    ...
    try {
      ...
      if (res.status === 429) {
        setMessages(prev => [...prev, { message: data.error || "⛔ Limite de messages atteinte.", sender: "ChatGPT" }]);
      } else {
        setMessages([...newMessages, { message: data.reply, sender: "ChatGPT" }]);
      }
    } catch {
      setMessages(prev => [...prev, { message: "❌ Une erreur est survenue. Réessaie plus tard.", sender: "ChatGPT" }]);
    } finally {
      setTyping(false);
    }
  };
        ...
        <MainContainer className="cs-main-container">
          <ChatContainer className="cs-chat-container">
            <MessageList
              ...
              typingIndicator={typing && <TypingIndicator content="Mawa est en train d’écrire..." />}
            >
              {messages.map((msg, i) => (
                <Message
                  key={i}
